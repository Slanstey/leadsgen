...
export function CsvUploadDialog({
  ...
  const optionalCompanyFields = [
    { key: "company_location", label: "Company Location", required: true },
    ...
    try {
      ...
      const processedData = parsedData
        .map((row) => {
          ...
          const companyLocation = columnMapping.company_location
      ...
      for (let i = 0; i < processedData.length; i += batchSize) {
          ...
          if (!companyMap.has(companyName)) {
            try {
              ...
              if (existingCompany) {
                ...
                if (companyError) {
                  console.error("[CSV Upload] Error creating company:", companyError);
                  console.error("[CSV Upload] Company error details:", {
                    message: companyError?.message,
                    code: companyError?.code,
                    details: companyError?.details,
                  });
                  // Continue anyway - we'll still try to create the lead
                } else if (newCompany) {
        ...
        // Now create leads
        const leadsToInsert = batch.map((item) => item.lead);
            ...
            if (leadsError.code === "23505") {
              ...
              for (const lead of leadsToInsert) {
                try {
                  ...
                  if (!existingLead) {
                    await supabase.from("leads").insert(lead).select();
                    leadsCreated++;
                  } else {
                    leadsSkipped++;
                  }
        ...
        <div className="space-y-6 py-4 flex-1 min-h-0 overflow-y-auto flex flex-col">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              Optional: Company Location, Industry, Sub-Industry, Annual Revenue, Description.
            </p>
          </div>
